...
#pragma omp section
    ...
    if (colisiones_totales < 5)
        printf("RESULTADO: SOBREVIVE\n");
    else if (colisiones_totales > 5)
        printf("RESULTADO: MUERE\n");
    else
        printf("RESULTADO: LIMITE (5 colisiones)\n");
}

int main(int argc, char **argv)
{
    // CAMBIO (input grande para medir speedup/eficiencia):
    // N_FRAMES y N_OBS aumentados para generar carga significativa.
    int N_FRAMES = 1000;
    int N_OBS = 200000;
    float JUMP_H = 1.2f;
    float PROB_SALTO = 0.25f;

    Obstacle *obs = (Obstacle *)malloc(sizeof(Obstacle) * N_OBS);
    ...
    // CAMBIO (medición de tiempo para speedup/eficiencia):
    double t0 = now_ms();
